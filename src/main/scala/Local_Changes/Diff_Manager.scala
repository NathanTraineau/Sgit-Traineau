    ...
    def loop(new_index_lines : List[String], index_line : String  ): Relationship ={
      ...
      if (new_index_lines.isEmpty) {
        ...
        if (sha1_1 == sha1_2 && name_1 == name_2){
          ...
          val former_blob_File_content: List[String] = Blob_Manager.get_blob_file_content(index_line_file)
          val new_blob_file_content: List[String] = Blob_Manager.get_blob_file_content(file_new_file_line)
          val list_changes: List[Change] = Diff_Manager.get_modif_files(new_blob_file_content,former_blob_File_content)
          Relationship(Some(file_new_file_line),Some(index_line_file),"modified",new_index_lines.head,list_changes)
        } else
        {
